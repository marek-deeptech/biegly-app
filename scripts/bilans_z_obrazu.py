...
from engine.sprawozdania import _liczba, pozycje_z_tabel  # noqa: E402
...
ODCZYTY: dict[str, list[Odczyt]] = {
    "SKOK": [
        ...
        # Raport EBI 8/2014 — informacja o adekwatności kapitałowej. Strona 1 to
        # rachunek wg art. 127 Prawa bankowego, strona 2 wg CRR: DWIE metodyki
        # funduszy własnych, których nie wolno zlać. Strona 22 to nota
        # klasyfikacyjna należności (mianownik NOMINALNY wskaźników jakości).
        Odczyt(
            "ebi14_08.ocr.pdf",
            [Strona(1, "pb"), Strona(2, "crr"), Strona(22)],
...
def czytaj_odczyt(pdf: pathlib.Path, odczyt: Odczyt, dpi: int) -> list[dict]:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        for s in odczyt.strony:
            ...
            if s.metodyka:
                # Metodyka opisuje RACHUNEK na stronie. Gdy strona dała więcej niż
                # jedną tabelę z wierszem funduszy własnych, przypisanie byłoby
                # domysłem — wtedy przerywamy, zamiast oznaczyć obie tak samo.
                z_funduszami = [t for t in tabele if any(
                    str(w.get("etykieta", "")).strip().lower().startswith("fundusze własne")
                    for w in (t.get("wiersze") or []))]
                if len(z_funduszami) > 1:
                    raise SystemExit(
                        f"✗ {odczyt.plik} str.{s.nr}: {len(z_funduszami)} tabele z wierszem "
                        f"„Fundusze własne”, a metodyka „{s.metodyka}” jest deklarowana dla całej "
                        "strony — przypisanie byłoby domysłem. Rozbij spis na osobne strony."
                    )
            for t in tabele:
                t["strona"] = s.nr
                t["plik"] = odczyt.plik
                ...
                if s.metodyka:
                    t["metodyka"] = s.metodyka
                out.append(t)
            print(f"   str.{s.nr:>3}: {len(tabele)} tab.  kolumny={tabele[0].get('kolumny') if tabele else '—'}"
                  f"{'  metodyka=' + s.metodyka if s.metodyka else ''}")
    return out
...
def kontrola_udzialow(tabele: list[dict]) -> list[str]:
    ...
    ⚠️ TO WALIDUJE WPROST LICZBY NAGŁÓWKOWE OPINII. Nota klasyfikacyjna podaje pod
    każdą datą kolumnę złotową i procentową; jeżeli 110 626 541,71 / 1 644 687 085,45
    daje 6,73 %, a dokument też pisze 6,73 %, to odczyt obu kolumn potwierdza się
    nawzajem. Rozbieżność jest cechą materiału albo błędem odczytu i MUSI być
    zgłoszona — sam iloraz tego nie wychwyci, bo liczy się z tych samych liczb.
    """
    ...
    for t in tabele:
        ...
        pary = [(i, i + 1) for i in range(len(kolumny) - 1)
                if "%" not in kolumny[i] and "%" in kolumny[i + 1]]
        for i_zl, i_pct in pary:
            def w_kolumnie(w: dict, i: int):
                war = w.get("wartosci") or []
                return liczba(war[i]) if i < len(war) else None

            # Mianownik to wiersz, któremu dokument przypisuje udział 100 % —
            # „Należności od sektora niefinansowego brutto” w nocie klasyfikacyjnej.
            suma = next((w_kolumnie(w, i_zl) for w in wiersze
                         if (u := w_kolumnie(w, i_pct)) is not None and abs(u - 100.0) < 0.005
                         and w_kolumnie(w, i_zl)), None)
            if not suma:
                continue
            for w in wiersze:
                kwota, udzial = w_kolumnie(w, i_zl), w_kolumnie(w, i_pct)
                if kwota is None or udzial is None or abs(udzial - 100.0) < 0.005:
                    continue
                policzony = 100.0 * kwota / suma
                if abs(policzony - udzial) > 0.02:
                    uwagi.append(
                        f"{t.get('plik')}, str. {t.get('strona')}, kolumna „{kolumny[i_zl]}”, "
                        f"wiersz „{str(w.get('etykieta'))[:40]}”: iloraz daje {policzony:.2f} %, "
                        f"a dokument podaje {udzial:.2f} % — rozbieżność w materiale albo "
                        "w odczycie; sprawdź w oryginale."
                    )
    return uwagi
