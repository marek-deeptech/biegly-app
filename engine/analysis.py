...
def compute_all(transactions: list[dict], orders: list[dict]) -> list[dict]:
    out: list[dict] = []
    ...
    gt = metrics.group_turnover_share(transactions)
    ...
    for d in days:
        w = metrics.wash_trade_share(transactions, d)
        out.append(_m(f"wash_{d}", "Wash-trades (udział w wolumenie sesji)",
                      round(w["share"] * 100, 2), "%", d))
        c = metrics.cancelled_buy_share(orders, owner_map, d)
