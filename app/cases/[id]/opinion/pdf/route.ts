...
// Pobranie opinii jako PDF w dopracowanej formie (jak analiza OSINT): IBM Plex Sans,
// spis treści z numeracją stron, banery rozdziałów I–VI, tabele, wykresy, stopka.
// Ten sam model (buildOpinion) co wariant .docx; render pdfmake (bez LibreOffice).
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: caseRow } = await supabase.from("cases").select("name,signature,group_roster,typ").eq("id", id).single();
  ...
  const fname = `Opinia_${safe}${final ? "" : "_projekt"}.pdf`;
